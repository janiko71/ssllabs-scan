...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="LabsReport/Endpoints/Details/*[.]">  
  ...
  <xsl:template match="LabsReport/Endpoints/*[.]">  
    <p><xsl:value-of select="name()" /> : <xsl:value-of select="." /></p>
  </xsl:template>
  ...
  <xsl:template match="LabsReport/Certs">  
  Certs
  </xsl:template>
      ...
      <xsl:otherwise>
        ...
        <xsl:choose>
          <xsl:when test="not(normalize-space(.))">
            ...
            <tr>
              <th>(b3)<xsl:value-of select="name()" /></th>
              <td><i>vide</i></td>
            </tr>
          </xsl:when>
          <xsl:otherwise>
            <!-- Singleton -->
            <tr>
              <th>(b4)<xsl:value-of select="name()" /></th>
              <td>(b5)<xsl:value-of select="." /></td>
            </tr>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:otherwise>
